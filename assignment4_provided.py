...
def handleOneClient(sock):
    ...
    print("supposed to see beforeLoginMsg and username prompt")
    ...
    time.sleep(0.1)
    print("supposed to see welcome message")

    cmdCount = 0
    mySendAll(sock, f"<{userName}:{cmdCount}> ".encode())
    print("supposed to see prompt for the command line")
    ...
    while True:
        ...
        if (command == 'quit' or command == 'exit'):
            ...
            print("supposed to see the goodbyeMsg")
            sock.close()
            break
        else: 
        ...
        cmdCount = cmdCount + 1
        mySendAll(sock, f"<{userName}:{cmdCount}> ".encode())
        print("supposed to see the command line prompt when looped")

if __name__ == "__main__":
    n = len(sys.argv)
    if (n != 2):
        print("Add argument: port number")
        exit()
